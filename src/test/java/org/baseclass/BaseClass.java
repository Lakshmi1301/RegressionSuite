...
//        public static void  record() throws ATUTestRecorderException
//        {  
...
//	     recorder = new ATUTestRecorder("C:/Recordings/","data_video" + dateFormat.format(date), true);
//
...
public static void brokenLink(List<WebElement> links ) throws IOException
{
	...
	URL u=new URL(a);
	URLConnection openURL = u.openConnection();
	HttpURLConnection h=(HttpURLConnection) openURL;
	int responseCode = h.getResponseCode();
	if(responseCode!=200)
	{
		System.out.println("error URL "  +a);
		count++;
	}
	
	}

	System.out.println("errorURL Count" +count);

        }
        

//broken image
public static void brokenImage(List<WebElement> images)throws IOException
{
int count=0;
for (WebElement img : images) {
	...
	int responseCode = h.getResponseCode();
	if(responseCode!=200) {
		System.out.println("error image "  +attribute);
		count++;
	}
}
System.out.println("error image" +count);
